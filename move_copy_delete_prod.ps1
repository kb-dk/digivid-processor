...
[string]$SAMBAshare="\\halley\vhsfile\"
...
function CopyCompareWren {
	...
	#Hvis filen skal flyttes, s� skal source slettes
	if ($Move) {
...
# Stopper Digivid processor, hvis det k�rer
#Stop-Process -Name java -Force 

# Flytter alle comments-filer med tilh�rende filer til windir2
...
#Kopierer alle comments-filer med tilh�rende transportstream-filer til SAMBAshare
foreach ($commentsFile in get-childitem -path $windir2 -filter *$commentsFileExt) {
	...
	# Hvis der ikke er en tilsvarende fil med doneFileExt, s� skal filerne kopieres
...
#Sletter alle comments-filer med tilh�rende transportstream-filer og donefilext �ldre end retainDays dage fra windir2
#dato for sidste �ndring i donefile
